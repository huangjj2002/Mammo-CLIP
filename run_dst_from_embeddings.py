...
def predict_to_frame(meta, features, labels, positions, model, criterion, args, device, progress_desc="predict"):
    loader = make_loader(
        features,
        labels,
        positions,
    ...
    with torch.no_grad():
        for batch_features, batch_labels, batch_rows in maybe_progress(loader, args, progress_desc):
            ...
            task_loss = criterion(head_output, batch_labels)
            proto_loss, proto_stats = prototype_regularization_loss(model, head_output, batch_labels, args)
            loss = task_loss + proto_loss
...
def run_single(args):
    ...
    for epoch in range(args.epochs):
        train_step_loss, train_step_stats = train_one_epoch(model, train_loader, criterion, optimizer, device, args)
    ...
    pred_df, _, _ = predict_to_frame(meta, features, labels, all_positions, model, criterion, args, device)
    ...
    history_path, loss_curve_path = save_training_history(history, output_dir, "dst", args, "DST")
