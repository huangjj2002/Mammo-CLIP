...
def _save_eval_predictions(args, oof_df):
    ...
    if args.n_folds > 0:
        ...
        aucroc_value = auroc(gt=agg[args.label].values.astype(int), pred=agg["prediction"].values)
        print(f"TEST AUC-ROC from best checkpoint: {aucroc_value:.4f}")
    ...
    if args.n_folds == 0:
        test_path = args.output_path / f"seed_{args.seed}_test_outputs.csv"
        oof_df.to_csv(test_path, index=False)
        print(f"Holdout test predictions also saved to: {test_path}")
