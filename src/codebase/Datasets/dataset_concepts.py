...
class MammoDataset_concept(Dataset):
    ...
    def __getitem__(self, idx):
        img_path = None
        study_id = None
        laterality = self.df.iloc[idx].get('laterality', 'L')
        if  self.target_dataset.lower() == 'rsna':
            study_id = self.df.iloc[idx].get('STUDY_ID', self.df.iloc[idx].get('patient_id'))
            img_path = self.dir_path / str(study_id) / str(self.df.iloc[idx].get('IMAGE_ID', self.df.iloc[idx].get('image_id')))
        ...
        y = None
        if self.target_dataset.lower() == 'rsna':
            y = torch.tensor(self.df.iloc[idx].get('cancer', 0), dtype=torch.long)
