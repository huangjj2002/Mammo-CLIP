...
try:
    ...
    log.info("nltk punkt already exists, skip download")
except LookupError:
    log.info("Download nltk punkt module")
    try:
        nltk.download("punkt", quiet=True)
    except Exception as e:
        log.warning(f"Failed to download nltk punkt: {e}")
        log.warning("Please manually download: python -c \"import nltk; nltk.download('punkt')\"")
